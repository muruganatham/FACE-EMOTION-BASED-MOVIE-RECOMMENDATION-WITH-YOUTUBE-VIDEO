...
movie_data = pd.read_csv('C:\\xampp\\xampp\\htdocs\\main project\\movie_dataset.csv')
@app.route('/')
def home():
...
def register():
    ...
    if request.method == 'POST' and all(key in request.form for key in ['first_name', 'last_name', 'email', 'password', 'confirm_password', 'phone']):
        ...
        if account:
            ...
            cursor.execute('INSERT INTO login (first_name, last_name, email, password, phone) VALUES (%s, %s, %s, %s, %s)',
...
def change_password():
    if request.method == 'POST':
        ...
        cursor = mydb.cursor(dictionary=True)
        cursor.execute('SELECT * FROM login WHERE email = %s AND password = %s', (email, current_password))
...
def get_movie_recommendations(emotion):
    ...
    recommended_movies_list = relevant_movies.sort_values(by='movie_rating', ascending=False).head(5)
